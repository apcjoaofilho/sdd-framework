...
- Replacing planning skills that already work (this framework *orchestrates* them, doesn't reimplement them).
- Becoming a full project management tool (no kanban, no time tracking, no team features).
- Locking users into a single AI vendor (every artifact must be readable by humans and by any agent).
...
| 3 | SPEC      | Discussion (or Research) | `openspec/changes/<id>/spec.md` + ADRs            | Functional + non-functional + acceptance criteria  |
| 4 | PLAN      | Spec                   | `.sdd/plans/YYYY-MM-DD-feature.md`                  | Tasks ≤5min, paths exact, copy-paste runnable      |
| 5 | EXECUTE   | Plan                   | Code, semantic commits, passing tests               | Per-task two-stage review (spec → quality)         |
...
| 9 | ARCHIVE   | Released code          | Spec moved to `openspec/archive/`, source-of-truth updated | Diff applied to `openspec/project.md`       |
...
|----|------|-------|------------|
| M1 | Extract templates to files (STATUS, STATE, AGENTS, ADR, spec-delta, project, gates) | — | Files exist, are valid, and pass `scripts/validate-templates.py` |
| M2 | Implement `scripts/check-status.py` | — | Reads `~/.sdd/projects.yaml`, emits well-formed JSON, has unit tests |
| M3 | STATE.json schema + validator | — | `references/state-schema.json` + JSON Schema validation in CI |
| M4 | Resolve artifact ownership (openspec/ vs .sdd/plans/ vs .agent/) | — | `references/portability-matrix.md` with diagram, no overlap unresolved |
